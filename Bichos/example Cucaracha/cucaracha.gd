...
@onready var impulse_point_upleft: Node3D = %"impulse point upleft"
@onready var impulse_point_upright: Node3D = %"impulse point upright"
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("roach_up_left"):
		apply_impulse(Vector3.UP * roach_flight_impulse, impulse_point_upleft.global_position - global_position)
		apply_impulse(roach_left_vector, impulse_point_left.global_position - global_position)
	if Input.is_action_just_pressed("roach_up_right"):
		apply_impulse(Vector3.UP * roach_flight_impulse, impulse_point_upright.global_position - global_position)
		apply_impulse(roach_right_vector, impulse_point_right.global_position - global_position)
